...
2. **単語を調査する**
   `google_web_search` を使って以下の情報を調査する：
   ...
   - `meanings`: 元の外国語単語の意味一覧。意味が複数ある場合はそれぞれ列挙し、用例があれば `example_en`・`example_ja` に記載する

3. **翻訳者を確認・追加する**
   訳語を作った翻訳者が判明した場合：
   - translators.jsonに既に存在すれば、そのIDを`translator_id`に使う
   - 存在しない場合は、`google_web_search` でその人物を調査し、translators.jsonに追加する：
     - `id`: 現在の最大ID+1
     - `name`: 人物名
   ...
   - さらに Wikimedia Commons で肖像画像を検索し（`https://commons.wikimedia.org/wiki/File:{名前}.jpg` など）、
     `run_shell_command` で `curl` を用いて `public/translators/` にダウンロードする。画像が見つからない場合は`image`フィールドを省略する。
